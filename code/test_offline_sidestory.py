        ...
        try:
            ...
            while not api["is_game_end"] and cnt <= 30:
                ...
                if interaction == "对话":
                    ...
                    # --- START OF MODIFICATION (New logic block) ---
                    action_to_take = None  # 用于存储优先选择的动作

                    # 检查当前场景是否为 "场景大街" 并且是否有 $支线 动作可选
                    if agent.curr_scene == "场景大街":
                        # 从 api["action_space"] 筛选出所有包含 "$支线" 的动作
                        zhixian_actions = [act for act in api["action_space"] if "$支线" in act]
                        if zhixian_actions:
                            # 如果有 $支线 动作，从中随机选择一个
                            action_to_take = random.choice(zhixian_actions)
                    

                    if action_to_take:  # 如果通过上面的新逻辑选择了 $支线 动作
                        action = action_to_take
                    # --- END OF MODIFICATION (New logic block) ---
